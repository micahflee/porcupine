!define APPNAME "Porcupine"
!define BINPATH "..\dist\porcupine"
!define ABOUTURL "https:\\github.com\micahflee\porcupine"

# change these with each release
!define INSTALLSIZE 36439
...
!else
    ...
    !system "makensis.exe /DINNER porcupine.nsi" = 0
    !system "$%TEMP%\tempinstaller.exe" = 2
    !system "signtool.exe sign /v /d $\"Uninstall Porcupine$\" /a /tr http://timestamp.globalsign.com/scripts/timstamp.dll /fd sha256 $%TEMP%\uninstall.exe" = 0

    # all done, now we can build the real installer
    OutFile "..\dist\Porcupine_Setup.exe"
...
Section "install"
    ...
    File "${BINPATH}\porcupine.exe"
    File "${BINPATH}\porcupine.exe.manifest"
    ...
    File "${BINPATH}\share\porcupine.desktop"
    File "${BINPATH}\share\porcupine.png"
    ...
    # start menu
    CreateShortCut "$SMPROGRAMS\${APPNAME}.lnk" "$INSTDIR\porcupine.exe" "" "$INSTDIR\icon.ico"
...
!ifdef INNER
    Section "uninstall"
        ...
        Delete "$INSTDIR\porcupine.exe"
        Delete "$INSTDIR\porcupine.exe.manifest"
        ...
        Delete "$INSTDIR\share\porcupine.desktop"
        Delete "$INSTDIR\share\porcupine.png"
